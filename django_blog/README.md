...
When running the app for the first time, use can use the provided start script. This will:
...
- A valid **SSL certificate** for `localhost`
  You can [generate a self-signed certificate](#-generating-local-certificates) for local testing
...
After successfully running the app it should now be available on **https://localhost:8443**.
